...
<h1 class="sr-only">Madagascar Green Tours - Blog List</h1>
...
<section id="about" class="container content-section bg-light" aria-labelledby="about-heading">
                <h1 class="sr-only">{{ $language == "es" ? "Experiencias culturales en Madagascar que no te puedes perder" : "Madagascar Sustainable Tourism: Explore Responsibly" }}</h1>
                <h2 class="sr-only">{{ $language == "es" ? "Experiencias culturales en Madagascar que no te puedes perder" : "Exploring Madagascar Sustainable Tourism Practices" }}</h2>
                        ...
                        <article class="card__styled card--1" onclick="window.location='{{ route('blogs/'.strtolower(str_replace(' ','-',$blog->title))) }}'">
                            <div class="card__info-hover">
                                ...
                                <div class="card__clock-info">
                                    ...
                                    </svg><span class="card__time">Shared on {{ $blog->created_at }}</span>
                                </div>

                            </div>
                            <div class="card__img"
                                style="background-image: url('{{ thumbnail(assets($blog->image)) }}');">
                            </div>
                            ...
                            <div class="card__info">
                                ...
                                <small class="text-muted">Posted on {{ formatDateTime($blog->created_at) }}</small>
